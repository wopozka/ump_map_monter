    ...
    def sprawdz_poprawnosc_klucza(self, dane_do_zapisu):
        for klucz in dane_do_zapisu:
            if klucz not in self.dozwolone_klucze:
                # Optymalizacja: startswith(tuple) jest znacznie szybszy niż pętla for z if startswith()
                # C-level implementation - O(1) zamiast O(n) dla każdego klucza
...
def montujpliki(args, naglowek_mapy=''):
    ...
    for pliki_w_ump, plik_ze_sciezka_pelna in plikidomont.pobierz_pliki_do_montowania():
        try:
            ...
            stderr_stdout_writer.stderrorwrite('Nie moge otworzyć pliku %s' % plik_ze_sciezka_pelna)
        else:
...
def stworz_plik_typ(args):
    ...
    Parameters
    ----------
    args: ArgParse(): argumenty linii poleceń
...
def main(argumenty):
    ...
    parser_montuj.add_argument('-sep', '--sprytne-entrypoints', action='store_true', default=False,
                               help='Ustaw EntryPoints jako Data0, dzięki czemu łatwiej się dodaje EP dla punktow')
